...
CachingReaderChunk::CachingReaderChunk(
        mixxx::SampleBuffer::WritableSlice sampleBuffer, mixxx::audio::ChannelCount channelCount)
        : m_index(kInvalidChunkIndex),
          m_sampleBuffer(std::move(sampleBuffer)),
          m_channelCount(channelCount) {
    DEBUG_ASSERT(m_sampleBuffer.length() == samples());
}
...
mixxx::IndexRange CachingReaderChunk::bufferSampleFrames(
    ...
    if (pAudioSource->getSignalInfo().getChannelCount() %
            ...
            0) {
        // This happens if the audio source only contain a mono channel
...
CachingReaderChunkForOwner::CachingReaderChunkForOwner(
        ...
        : CachingReaderChunk(std::move(sampleBuffer), kMaxSupportedChannels),
          m_state(FREE),
          m_pPrev(nullptr),
          m_pNext(nullptr) {
}
